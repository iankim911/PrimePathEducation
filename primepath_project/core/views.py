...
def exam_mapping(request):
    ...
    # === PHASE 2: EXAM PROCESSING (FIXED) ===
    # Get all active exams as QuerySet (keeping as objects, not converting to dicts prematurely)
    all_exams = Exam.objects.filter(is_active=True).order_by('name')
    ...
    # === PHASE 3: PROCESS PROGRAMS AND LEVELS ===
    for program in programs:
    ...
    # === PHASE 4: FINAL LOGGING AND VALIDATION ===
    if filtered_subprograms:
        log_filtered_subprograms(logger, filtered_subprograms)
