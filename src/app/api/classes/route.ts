import { listClassesByAcademy, createClass, type Class } from '@/lib/services/classes'
...
export async function GET() {
  try {
    ...
    const classes = await listClassesByAcademy(academyId)
...
export async function POST(request: Request) {
  try {
    ...
    const classData: Omit<Class, 'id' | 'created_at' | 'updated_at' | 'deleted_at'> = {
      ...
      curriculum_node_id: body.curriculum_node_id || null,
